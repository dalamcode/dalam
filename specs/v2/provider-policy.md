...
Both `action` and `resource` use dalam's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.dalam` files is intentionally deferred until `.dalam` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether dalam permits an operation through its managed execution paths.
