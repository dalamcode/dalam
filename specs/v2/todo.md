...
The dalam server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@uthakkan/llm` tool loop after replacing its
...
- plugins get global "dalam" instance like in that post i showed
- dalam instance has stuff like `dalam.session.prompt()` or
  `dalam.tool.register({...})`
...
- decide whether to preserve deprecated `@uthakkan/llm` orchestration exports
