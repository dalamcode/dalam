...
Move the canonical Dalam terminal application from
`packages/dalam/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/dalam`,
`packages/cli`, or `@uthakkan/core`.

The SDK is the TUI's Dalam boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/dalam`.
- Do not replace private `packages/dalam` imports with `@uthakkan/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@uthakkan/tui`.
...
- Dalam domain data displayed by the TUI
...
```text
packages/dalam/src/cli/cmd/tui
```

Its private dependency on `packages/dalam` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/dalam/src/*`.
...
Status: Completed. The private `@uthakkan/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@uthakkan/tui`.
...
- The package has no dependency on `dalam`, `@uthakkan/cli`, or
  `@uthakkan/core`.
...
primitives now live in `@uthakkan/tui`. The legacy host consumes explicit
...
and build inputs through `@uthakkan/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Dalam globals or process state;
...
Replace process-global Dalam state with resolved TUI inputs.
...
`@uthakkan/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/dalam` initially.
...
canonical in `@uthakkan/tui`. Configured references resolve through the new
...
adapters rather than Dalam domain implementations.

Make the SDK the only Dalam domain boundary used by the TUI.
...
- Domain-facing TUI code imports Dalam data and operations only from
  `@uthakkan/sdk`.
...
`@uthakkan/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/dalam`.
...
- The package has no imports from `packages/dalam`, `packages/cli`, or
  `@uthakkan/core`.
...
public `@uthakkan/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/dalam`.
- Keep the legacy embedded worker and server startup in `packages/dalam`.
- Change those adapters to load config, create transport inputs, and call the
  public `@uthakkan/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/dalam`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@uthakkan/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@uthakkan/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Dalam domain state.
...
cd packages/dalam && bun typecheck
...
rg '@uthakkan/core|packages/dalam|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/dalam/src packages/dalam/test
```
...
- Build the current-platform `packages/dalam` binary.
