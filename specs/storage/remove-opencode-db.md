# Remove `packages/dalam/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/dalam/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the dalam legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/dalam/src/storage/db.ts` are concentrated in 22 source files:

- `packages/dalam/src/account/repo.ts`
- `packages/dalam/src/cli/cmd/db.ts`
- `packages/dalam/src/cli/cmd/import.ts`
- `packages/dalam/src/cli/cmd/stats.ts`
- `packages/dalam/src/control-plane/workspace.ts`
- `packages/dalam/src/index.ts`
- `packages/dalam/src/node.ts`
- `packages/dalam/src/permission/index.ts`
- `packages/dalam/src/project/project.ts`
- `packages/dalam/src/server/projectors.ts`
- `packages/dalam/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/dalam/src/server/shared/fence.ts`
- `packages/dalam/src/session/message-v2.ts`
- `packages/dalam/src/session/projectors.ts`
- `packages/dalam/src/session/prompt.ts`
- `packages/dalam/src/session/session.ts`
- `packages/dalam/src/session/todo.ts`
- `packages/dalam/src/share/share-next.ts`
- `packages/dalam/src/storage/db.ts`
- `packages/dalam/src/sync/index.ts`
- `packages/dalam/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy dalam database wrapper; `packages/dalam/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the dalam wrapper.
...
Status: Completed. `SyncEvent` and the dalam projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy dalam database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy dalam database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy dalam database wrapper.
...
All migration groups are complete or superseded. `packages/dalam/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No dalam data-migration group remains.

The previous dalam `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate dalam data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/dalam`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/dalam/src`
- `bun typecheck` from `packages/dalam`
- Relevant package tests from `packages/dalam`, not the repo root
