...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".dalam/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".dalam/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".dalam/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "dalam/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "dalam/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "dalam/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
