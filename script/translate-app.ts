...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "dalam/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".dalam/glossary/zh-cn.md"
  if (locale === "zht") return ".dalam/glossary/zh-tw.md"
  return `.dalam/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Dalam did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Dalam returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Dalam session export omitted the assistant model.")
    }
    return [
      {
...
export function translationConfig(agent: string, model: string, targets: string[]) {
  return {
    $schema: "https://dalam.ai/config.json",
    model,
    default_agent: agent,
...
Options:
  -c, --concurrency <count>  Maximum parallel Dalam runs for 'all' (default: 4)
      --model <provider/id>  Dalam model (default: dalam/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Dalam
...
`)
  ...
  if (failed.length) console.error(`\nDalam failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.DALAM_DISABLE_PROJECT_CONFIG = "1"
  env.DALAM_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["dalam", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.DALAM_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["dalam", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.DALAM_CONFIG
  delete env.DALAM_CONFIG_DIR
  delete env.DALAM_CONFIG_CONTENT
  delete env.DALAM_PERMISSION
  delete env.DALAM_AUTO_SHARE
  return env
}
