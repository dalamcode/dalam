...
const model = "dalam/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/dalam")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with dalam...`)
  ...
  try {
    await $`dalam run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  dalam failed: ${String(err)}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with dalam")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with dalam...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/dalam`.",
