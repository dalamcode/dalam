...
export function summarizeNavigationMilestones(samples: NavigationMilestoneSample[]) {
  ...
  return {
    ...
    all: summarize((sample) => names.every((name) => void sample.milestones[name] === true)),
  }
}
    ...
    ({ triggerSelector, milestones }) => {
          ...
          setTimeout(() => {
            ...
            Object.entries(current).forEach(([name, value]) => {
              ...
              if (!marked.has(`${name}.first`)) {
                performance.mark(`dalam.navigation.${name}.first`)
                marked.add(`${name}.first`)
              }
              const streak = (streaks.get(name) ?? 0) + 1
              streaks.set(name, streak)
              if (streak === 3) performance.mark(`dalam.navigation.${name}.stable`)
            })
            ...
            if (all && !marked.has("all.first")) {
              performance.mark("dalam.navigation.all.first")
              marked.add("all.first")
            }
            if (allStreak === 3) performance.mark("dalam.navigation.all.stable")
            sample()
          }, 0)
        })
      }
      document.addEventListener(
        ...
        (event) => {
          ...
          performance.mark("dalam.navigation.click")
          sample()
        },
