import { base64Encode } from "@uthakkan/core/util/encode"
import { Event } from "@uthakkan/schema/event"
import { SessionStatusEvent } from "@uthakkan/schema/session-status-event"
import { SessionV1 } from "@uthakkan/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockDalamServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/Dalam/TimelineStability"
...
export const model = { providerID: "dalam", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("dalam.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["dalam"],
    default: { providerID: "dalam", modelID: "claude-opus-4-6" },
  }
}
