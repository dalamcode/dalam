...
Set `DALAM_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
DALAM_PERFORMANCE_TRACE_DIR=/tmp/dalam-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `DALAM_PERFORMANCE_SELECTOR_TRACE=1`.
