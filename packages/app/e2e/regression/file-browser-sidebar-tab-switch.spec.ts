...
const directory = "C:/Dalam/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockDalamServer(page, {
    ...
    provider: {
      ...
      connected: ["dalam"],
      default: { providerID: "dalam", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "dalam.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "dalam.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "dalam.window.browser.dat:tabs",
