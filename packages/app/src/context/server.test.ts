  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://dalam.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://dalam.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://dalam.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://dalam.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://dalam.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://dalam.example.com"),
      ),
    ).toEqual({
