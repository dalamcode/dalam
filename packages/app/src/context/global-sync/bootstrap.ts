import type {
  ...
  ReferenceInfo,
  Session,
} from "@uthakkan/sdk/v2/client"
import { showToast } from "@/utils/toast"
import { getFilename } from "@uthakkan/core/util/path"
import { retry } from "@uthakkan/core/util/retry"
...
import { NormalizedProviderListResponse } from "@uthakkan/session-ui/context"
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: DalamClient) =>
  queryOptions({
    queryKey: [scope, "config"],
    queryFn: () => retry(() => sdk.global.config.get().then((x) => x.data!)),
  })

export const loadProjectsQuery = (scope: ServerScope, sdk: DalamClient) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        sdk.project.list().then((x) => {
          return (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("dalam-test"))
            .slice()
...
export const loadProvidersQuery = (scope: ServerScope, directory: string | null, sdk: DalamClient) =>
  queryOptions({
    queryKey: [scope, directory, "providers"],
    queryFn: () => retry(() => sdk.provider.list().then((x) => normalizeProviderList(x.data!))),
  })

export const loadAgentsQuery = (scope: ServerScope, directory: string | null, sdk: DalamClient) =>
  queryOptions({
    queryKey: [scope, directory, "agents"],
    queryFn: () => retry(() => sdk.app.agents().then((x) => normalizeAgentList(x.data))),
  })

export const loadPathQuery = (scope: ServerScope, directory: string | null, sdk: DalamClient) =>
  queryOptions<Path>({
    queryKey: [scope, directory, "path"],
    queryFn: () => retry(() => sdk.path.get().then((x) => x.data!)),
  })

export const loadReferencesQuery = (scope: ServerScope, directory: string, sdk: DalamClient) =>
