...
import type { SessionNotFoundError } from "@uthakkan/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em dalam.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
