...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("dalam://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("dalam://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("dalam://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("dalam://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("dalam://open-project")).toBeUndefined()
    expect(parseDeepLink("dalam://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "dalam://open-project?directory=/a",
      "dalam://other?directory=/b",
      "dalam://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("dalam://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("dalam://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("dalam://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("dalam://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "dalam://new-session?directory=/a",
      "dalam://open-project?directory=/b",
      "dalam://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __DALAM__: {
        deepLinks: ["dalam://open-project?directory=/a"],
      },
    } as unknown as Window & { __DALAM__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["dalam://open-project?directory=/a"])
