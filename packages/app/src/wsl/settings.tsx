...
import { Tag } from "@uthakkan/ui/v2/badge-v2"
...
import { wslDalamAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.dalamChecks[item.config.distro]
          const dalamAction = () => wslDalamAction(check())
          const busy = () => wsl.data?.job?.kind === "install-dalam" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={dalamAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installDalam(item.config.distro))}
                    >
