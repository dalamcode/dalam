...
import type { DalamEvent } from "../src"

// Set :memory: before any dynamic import of ../src so database.ts uses it
...
test("embedded client uses the real router and handlers", async () => {
  ...
  try {
    const program = Effect.gen(function* () {
      const dalam = yield* Dalam.create()
      yield* dalam.tools.register({
      ...
      const created = yield* dalam.sessions.create({
        id: sessionID,
        agent: Agent.ID.make("build"),
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      yield* dalam.sessions.switchModel({ sessionID, model })
      const selected = yield* dalam.sessions.get({ sessionID })
      const page = yield* dalam.sessions.list({ directory: AbsolutePath.make(directory) })
      const active = yield* dalam.sessions.active()
      const admitted = yield* dalam.sessions.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Do not run" }),
        resume: false,
      })
      const context = yield* dalam.sessions.context({ sessionID })
      const wake = yield* dalam.sessions.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Promote this input" }),
      })
      const prompted = yield* dalam.sessions.events({ sessionID }).pipe(
        Stream.filter((event) => event.type === "session.next.prompted" && event.data.messageID === wake.id),
        Stream.runHead,
        Effect.timeout("10 seconds"),
        Effect.map(Option.getOrThrow),
      )
      const wakeContext = yield* dalam.sessions.context({ sessionID })
      const event = yield* dalam.sessions
        .events({ sessionID })
        .pipe(Stream.take(1), Stream.runHead, Effect.map(Option.getOrUndefined))
      const modelMessage = Option.fromNullishOr(context.find((message) => message.type === "model-switched")).pipe(
        Option.getOrThrow,
      )
      const message = yield* dalam.sessions.message({ sessionID, messageID: modelMessage.id })
      yield* dalam.sessions.interrupt({ sessionID })
      const other = yield* dalam.sessions.create({
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      const missingSessionID = Session.ID.make(`ses_missing_${crypto.randomUUID()}`)
      const missing = yield* Effect.all(
        [
          dalam.sessions.events({ sessionID: missingSessionID }).pipe(Stream.runHead, Effect.flip),
          dalam.sessions.interrupt({ sessionID: missingSessionID }).pipe(Effect.flip),
          dalam.sessions.message({ sessionID: missingSessionID, messageID: modelMessage.id }).pipe(Effect.flip),
        ],
        { concurrency: "unbounded" },
      )
      const missingMessage = yield* Effect.flip(
        dalam.sessions.message({
          sessionID: other.id,
    ...
    await rm(directory, { recursive: true, force: true } as any)
  }
})
...
test("Location-owned runner events reach the ready global client", async () => {
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      const prompted = yield* Deferred.make<DalamEvent>()
      yield* dalam.events.subscribe().pipe(
      ...
      yield* dalam.sessions.create({
        id: sessionID,
        location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
      })
      yield* dalam.sessions.prompt({ sessionID, prompt: Prompt.make({ text: "Observe this input" }) })
...
test("independent embedded hosts do not share live notifications", async () => {
  ...
  try {
    const program = Effect.gen(function* () {
      const first = yield* Dalam.create()
      const second = yield* Dalam.create()
      ...
      const observe = (ready: Latch.Latch, event: Latch.Latch) =>
        Stream.runForEach((notification: DalamEvent) =>
...
test("embedded client is available as a Layer service", async () => {
  ...
  try {
    const created = await Effect.runPromise(
      Effect.gen(function* () {
        const dalam = yield* Dalam.Service
        return yield* dalam.sessions.create({
          id: sessionID,
          location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
        })
      }).pipe(Effect.provide(Dalam.layer), Effect.scoped),
    )
