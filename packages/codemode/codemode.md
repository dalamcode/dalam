...
This is the living design and status document for `@uthakkan/codemode` and its existing V2 Dalam adapter.
...
`packages/dalam/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
