...
import { PromptInput } from "@uthakkan/schema/prompt-input"
import { Session } from "@uthakkan/schema/session"
import { Project } from "@uthakkan/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@uthakkan/schema/schema"
...
import { SessionEvent } from "@uthakkan/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Dalam process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Dalam process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
