import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@uthakkan/plugin/tui"
...
import type { useDalamKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useDalamKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getDalamModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getDalamModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
