...
export const DALAM_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const DALAM_MODE_KEY = "dalam.mode"

export const DalamKeymapProvider = KeymapProvider
export const useDalamKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type DalamModeStack = ReturnType<typeof createDalamModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, DalamModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createDalamModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(DALAM_MODE_KEY, DALAM_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(DALAM_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(DALAM_MODE_KEY, stack.at(-1)?.mode ?? DALAM_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? DALAM_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(DALAM_MODE_KEY, undefined)
...
export function useDalamModeStack() {
  return getDalamModeStack(useDalamKeymap())
}

export function getDalamModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Dalam mode stack is not registered for this keymap")
  return value
}
...
export function registerDalamKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createDalamModeStack(keymap)
