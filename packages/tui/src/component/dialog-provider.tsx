...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@uthakkan/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__dalam_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in dalam.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={() =>
        ({
          dalam: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Dalam gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in dalam.json to use it.`,
          })
          dialog.clear()
          return
        }
