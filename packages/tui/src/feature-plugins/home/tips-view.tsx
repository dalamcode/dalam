import type { TuiPluginApi } from "@uthakkan/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("dalam.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}dalam.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/dalam/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.dalam/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.dalam/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.dalam/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.dalam/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Dalam from reading sensitive files",
  "Use {highlight}dalam run{/highlight} for non-interactive scripting",
  "Use {highlight}dalam --continue{/highlight} to resume the last session",
  "Use {highlight}dalam run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}dalam serve{/highlight} for headless API access to Dalam",
  "Use {highlight}dalam run --attach{/highlight} to connect to a running server",
  "Run {highlight}dalam upgrade{/highlight} to update to the latest version",
  "Run {highlight}dalam auth list{/highlight} to see all configured providers",
  "Run {highlight}dalam agent create{/highlight} for guided agent creation",
  "Use {highlight}/dalam{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}dalam github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/dalam fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.dalam/themes/{/highlight} directory",
  ...
  "Run {highlight}dalam debug config{/highlight} to troubleshoot configuration",
