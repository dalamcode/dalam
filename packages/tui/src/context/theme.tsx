...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      directories.push(path.join(current, ".dalam"))
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "dalam")
        draft.active = typeof active === "string" ? active : "dalam"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "dalam")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.dalam, store.mode)
    })
