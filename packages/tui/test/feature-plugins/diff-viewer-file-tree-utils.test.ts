...
describe("diff viewer file tree utilities", () => {
  ...
  test("collapses unary directory chains while flattening", () => {
    const rows = flattenFileTree(
      buildFileTree([{ file: "packages/dalam/src/cli/app.ts" }, { file: "packages/dalam/src/server/server.ts" }]),
    )

    expect(rows.map((row) => `${"  ".repeat(row.depth)}${row.kind}:${row.name}`)).toEqual([
      "directory:packages/dalam/src",
      "  directory:cli",
      "    file:app.ts",
      "  directory:server",
      "    file:server.ts",
  ...
  test("does not collapse a directory into a file row", () => {
    const rows = flattenFileTree(buildFileTree([{ file: "packages/dalam/src/app.ts" }]))
  ...
  test("stops collapsing at branches", () => {
    const rows = flattenFileTree(
      buildFileTree([
        { file: "packages/dalam/src/cli/app.ts" },
        { file: "packages/dalam/src/server/server.ts" },
  ...
  test("collapses expanded unary children under the first visible directory id", () => {
    ...
    expect(flattenFileTree(tree, new Set()).map((row) => row.name)).toEqual(["packages/dalam/src"])
  ...
  test("moves collapsed chain selection to first visible child", () => {
    ...
    const packages = rows.find((row) => row.kind === "directory" && row.name === "packages/dalam/src")!
  ...
  test("moves file and collapsed directory selection to visible parent", () => {
    ...
    const root = rows.find((row) => row.kind === "directory" && row.name === "packages/dalam/src")!
    const cli = rows.find((row) => row.kind === "directory" && row.name === "cli")!
