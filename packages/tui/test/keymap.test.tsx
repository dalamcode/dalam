...
import { getDalamModeStack, DALAM_BASE_MODE, DalamKeymapProvider, registerDalamKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerDalamKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when dalam mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getDalamModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getDalamModeStack(keymap).push("autocomplete")
