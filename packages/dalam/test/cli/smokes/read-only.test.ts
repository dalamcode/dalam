...
describe("dalam read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // DALAM_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ dalam }) =>
      Effect.gen(function* () {
        const r = yield* dalam.spawn(["mcp", "list"])
        dalam.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dalam }) =>
      Effect.gen(function* () {
        const r = yield* dalam.spawn(["providers", "list"])
        dalam.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ dalam }) =>
      Effect.gen(function* () {
        const r = yield* dalam.spawn(["models"])
        dalam.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ dalam }) =>
      Effect.gen(function* () {
        const r = yield* dalam.spawn(["agent", "list"])
        dalam.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh DALAM_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ dalam }) =>
      Effect.gen(function* () {
        const r = yield* dalam.spawn(["session", "list"])
        dalam.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dalam }) =>
      Effect.gen(function* () {
        const r = yield* dalam.spawn(["stats"])
        dalam.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under DALAM_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ dalam }) =>
      Effect.gen(function* () {
        const r = yield* dalam.spawn(["db", "path"])
        dalam.expectExit(r, 0, "db path")
