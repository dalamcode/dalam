...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ dalam }) =>
    Effect.gen(function* () {
      const result = yield* dalam.spawn(["--replay-limit", "10"])

      dalam.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ dalam }) =>
    Effect.gen(function* () {
      const result = yield* dalam.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ dalam }) =>
    Effect.gen(function* () {
      const result = yield* dalam.spawn(["--mini", "--port", "4096"])
