...
`,
  ...
  const pure = process.env.DALAM_PURE
  const meta = process.env.DALAM_PLUGIN_META_FILE
  process.env.DALAM_PURE = "1"
  process.env.DALAM_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.DALAM_PURE
    } else {
      process.env.DALAM_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.DALAM_PLUGIN_META_FILE
    } else {
      process.env.DALAM_PLUGIN_META_FILE = meta
    }
  }
})
