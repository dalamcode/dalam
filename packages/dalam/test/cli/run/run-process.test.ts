// Subprocess integration tests for `dalam run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `dalam.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `DALAM_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("dalam run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dalam.run("say hi")
        dalam.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dalam.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* dalam.run("think", { extraArgs: ["--thinking"] })
        dalam.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* dalam.run("think again")
        dalam.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dalam.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dalam.run("say hi", { format: "json" })
        dalam.expectExit(result, 0)

        const events = dalam.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ dalam }) =>
      Effect.gen(function* () {
        const result = yield* dalam.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dalam.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dalam.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* dalam.run("request permission", { permission: { bash: "ask" } })
        dalam.expectExit(denied, 0)
        ...
        const allowed = yield* dalam.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        dalam.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* dalam.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        dalam.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, dalam }) =>
      Effect.gen(function* () {
        ...
        const server = yield* dalam.serve()

        const result = yield* dalam.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, dalam }) =>
      Effect.gen(function* () {
        const result = yield* dalam.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, dalam }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* dalam.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
