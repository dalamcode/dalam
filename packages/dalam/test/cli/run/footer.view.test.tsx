...
import type { QuestionRequest } from "@uthakkan/sdk/v2"
import { DalamKeymapProvider, registerDalamKeymap } from "@uthakkan/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerDalamKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerDalamKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "dalam", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerDalamKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "dalam", modelID: "gpt-5" })
