...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/dalam.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/dalam.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/dalam.jsonc", dir: ".dalam", suggestion: "dalam" },
        expected:
          'Directory ".dalam" in /tmp/dalam.jsonc is not valid. Rename the directory to "dalam" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/dalam.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/dalam.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `dalam models` to list available models",
      "Or check your config (dalam.json) provider/model names",
    ].join("\n")
