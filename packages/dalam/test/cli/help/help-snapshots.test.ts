...
// terminal sizes. The default dalam tui command is excluded —
// `dalam --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `dalam --help` prints today;
...
// top-level help on `--help` and exits 1; not a real dalam command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("dalam CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ dalam }) =>
      Effect.gen(function* () {
        const topLevel = yield* dalam.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* dalam.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`dalam ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`dalam ${argv.join(" ")} --help`)
        }
