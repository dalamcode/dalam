// Subprocess test harness for the dalam CLI. Spawns the real binary against
...
//
// Configuration flows through dalam's built-in test affordances:
//   - DALAM_CONFIG_CONTENT      : provider config inline, no files to find
//   - DALAM_TEST_HOME           : pins os.homedir() → tmpdir
//   - DALAM_DISABLE_PROJECT_CONFIG : skip walking up for dalam.json
//   - DALAM_PURE                : skip external plugin discovery + install
//   - DALAM_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
const dalamRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(dalamRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    DALAM_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    DALAM_CONFIG_CONTENT: configJson,
    DALAM_DISABLE_PROJECT_CONFIG: "1",
    DALAM_PURE: "1",
    DALAM_DISABLE_AUTOUPDATE: "1",
    DALAM_DISABLE_AUTOCOMPACT: "1",
    DALAM_DISABLE_MODELS_FETCH: "1",
    DALAM_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `dalam run`. New flags should
...
// `dalam serve` is a long-lived process — it never exits on its own.
...
// `dalam acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type DalamCli = {
  ...
  // Spawn `dalam serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `dalam acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("dalam.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          DALAM_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("dalam.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("dalam.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "dalam server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `dalam serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("dalam.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    // and hit endpoints on `dalam.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "dalam") {
...
// every call site. Use this for any test that needs the dalam CLI fixture.
//
...
// resources (e.g. `dalam.serve`) without an extra `Effect.scoped` wrapper —
