...
it.instance("plan agent denies edits except .dalam/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".dalam/plans/foo.md", plan.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".dalam", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.DALAM_TEST_HOME
      process.env.DALAM_TEST_HOME = test.directory
