...
import { LLMEvent, ToolFailure } from "@uthakkan/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@uthakkan/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("dalam"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("dalam") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("dalam") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, dalam, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored dalam auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("dalam") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("dalam"),
          options: { apiKey: "console-token" },
          key: "dalam-token",
        },
        auth: { type: "api", key: "dalam-token" },
      }),
    ).toMatchObject({
      type: "supported",
