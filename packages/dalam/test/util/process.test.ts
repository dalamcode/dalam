...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.DALAM_TEST ?? "")'), {
      env: {
        DALAM_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "DALAM_TEST_SHELL"], {
      shell: true,
      env: {
        DALAM_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("DALAM_TEST_SHELL=ok")
  })
