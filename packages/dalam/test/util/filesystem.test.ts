...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "dalam.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "dalam.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "dalam.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "dalam.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["dalam.json", "dalam.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "dalam.json"),
        path.join(project, "dalam.jsonc"),
      ])
    })
  })
