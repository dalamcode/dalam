 ...
 *
 * The script intentionally isolates `DALAM_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "dalam.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "dalam.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-dalam-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-dalam-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "dalam", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-dalam-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "content-type": "application/json" },
      body: { model: { providerID: "dalam", id: "big-pickle" } },
    }))
    .status(204, undefined, "none"),
  http.protected
