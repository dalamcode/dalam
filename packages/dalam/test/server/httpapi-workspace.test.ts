...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-dalam-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-dalam-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://dalam.uthakkan.in/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-dalam-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-dalam-workspace")
