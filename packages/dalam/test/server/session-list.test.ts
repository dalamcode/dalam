  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "dalam"), { recursive: true }))
        ...
        const current = yield* withSession({ title: "current" }).pipe(
          provideInstance(path.join(test.directory, "packages", "dalam")),
        )
    ...
    () =>
      Effect.gen(function* () {
        ...
        const ids = (yield* SessionNs.Service.use((session) =>
          session.list({ directory: path.join(test.directory, "packages", "dalam") }),
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const test = yield* TestInstance
        const dir = path.join(test.directory, "packages", "dalam")
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "dalam", "src", "deep"), { recursive: true }),
        )
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "app"), { recursive: true }))
        ...
        const current = yield* withSession({ title: "current" }).pipe(
          provideInstance(path.join(test.directory, "packages", "dalam", "src")),
        )
        const deeper = yield* withSession({ title: "deeper" }).pipe(
          provideInstance(path.join(test.directory, "packages", "dalam", "src", "deep")),
        )
        const sibling = yield* withSession({ title: "sibling" }).pipe(
          provideInstance(path.join(test.directory, "packages", "app")),
        )
        ...
        if (process.platform === "win32") {
          const windowsPathIDs = (yield* SessionNs.Service.use((session) =>
            session.list({ path: "packages\\dalam\\src" }),
          )).map((session) => session.id)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "dalam", "src"), { recursive: true }),
        )
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "app"), { recursive: true }))
        ...
        const pathIDs = (yield* SessionNs.Service.use((session) =>
          session.list({
            directory: path.join(test.directory, "packages", "dalam", "src"),
            path: "packages/dalam/src",
          }),
        )).map((session) => session.id)
        expect(pathIDs).toContain(current.id)
