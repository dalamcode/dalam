...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const dalamFile = path.join(tmp, ".git", "dalam")
      expect(yield* Effect.promise(() => Bun.file(dalamFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "dalam")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "dalam")
      const wrongCache = path.join(parentDir, ".git", "dalam")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "dalam")
      const cacheB = path.join(bareB, "dalam")
      const wrongCache = path.join(parentDir, ".git", "dalam")
