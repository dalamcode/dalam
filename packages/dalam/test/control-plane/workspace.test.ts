...
import { EventSequenceTable } from "@uthakkan/core/event/sql"
...
import { WorkspaceV2 } from "@uthakkan/core/workspace"
import { WorkspaceTable } from "@uthakkan/core/control-plane/workspace.sql"
...
const originalEnv = {
  DALAM_AUTH_CONTENT: process.env.DALAM_AUTH_CONTENT,
  DALAM_EXPERIMENTAL_WORKSPACES: process.env.DALAM_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.DALAM_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@dalam.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.DALAM_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=dalam-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.DALAM_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.DALAM_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.DALAM_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=dalam-test")
