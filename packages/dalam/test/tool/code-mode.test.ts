...
describe("code mode execute", () => {
  test("defines execute input with an Effect schema", async () => {
    const decode = Schema.decodeUnknownEffect(Parameters)
     expect(Effect.runPromise(decode({ code: "return 1" }))).resolves.toEqual({ code: "return 1" })
  ...
  test("leaves oversized results to Dalam's native tool-output truncation", async () => {
    const tool = await build({})
