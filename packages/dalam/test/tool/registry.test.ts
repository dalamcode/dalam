...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".dalam")])),
})
...
describe("tool.registry", () => {
  ...
  withCodeMode.instance("exposes execute when code mode is enabled", () =>
    Effect.gen(function* () {
      ...
      const tools = yield* registry.tools({
        providerID: ProviderV2.ID.dalam,
  ...
  it.instance("loads tools from .dalam/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const dalam = path.join(test.directory, ".dalam")
      const tool = path.join(dalam, "tool")
  ...
  it.instance("ignores non-tool exports in .dalam/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".dalam", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-dalam` was registering a tool
  ...
  it.instance("loads tools from .dalam/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const dalam = path.join(test.directory, ".dalam")
      const tools = path.join(dalam, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".dalam", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const dalam = path.join(test.directory, ".dalam")
        const customTools = path.join(dalam, "tools")
        const plugin = path.join(dalam, "node_modules", "@uthakkan-dalam", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(dalam, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@uthakkan/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@uthakkan/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".dalam", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(dalam, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@uthakkan/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(dalam, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(dalam, "node_modules", "cowsay")
