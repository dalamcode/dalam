    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".dalam/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("uses the SDK package namespace rather than provider ID", () => {
    ...
    const result = ProviderTransform.message(msgs, dalamModel, { store: false }) as any[]
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const dalamModel = {
      ...
      api: {
        id: "dalam-test",
        url: "https://api.dalam.uthakkan.in",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    expect(result[0].content[0].providerOptions?.dalam?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.dalam?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.dalam?.itemId).toBe("msg_dalam")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.dalam?.itemId).toBe("msg_dalam_part")
