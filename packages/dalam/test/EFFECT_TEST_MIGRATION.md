...
it.instance("instance-local behavior", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("dalam-test-")
  }),
)
...
- `it.effect(...)` — pure Effect behavior with `TestClock` and
  `TestConsole`.
- `it.instance(...)` — service behavior that needs one scoped dalam
  instance.
...
9. Run the focused test file and `bun typecheck` from `packages/dalam`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/dalam/test
```
