...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.DALAM_TEST_HOME
      process.env.DALAM_TEST_HOME = home
      return prev
    }),
    () => self,
    (prev) =>
      Effect.sync(() => {
        process.env.DALAM_TEST_HOME = prev
      }),
  )
...
describe("skill", () => {
  ...
  it.live("discovers skills from .dalam/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".dalam", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".dalam", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".dalam", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .dalam/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".dalam", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".dalam", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".dalam", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".dalam", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".dalam", "skill", "dalam-skill", "SKILL.md"),
                `---
name: dalam-skill
description: A skill in the .dalam/skill directory.
---
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => s.location !== "<built-in>")
          expect(list.map((s) => s.name)).toEqual(["dalam-skill"])
        }),
      { git: true },
    ),
  )
...
`,
              ...
              Bun.write(
                path.join(dir, ".dalam", "skill", "agent-skill", "SKILL.md"),
                `---
name: dalam-skill
description: A skill in the .dalam/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".dalam", "skills", "agent-skill", "SKILL.md"),
                `---
name: dalam-skill
description: A skill in the .dalam/skills directory.
---
