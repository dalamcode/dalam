...
- Author package entrypoint is `@uthakkan-dalam/plugin/tui`.
...
```json
{
  "$schema": "https://dalam.uthakkan.in/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/dalam-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@uthakkan-dalam/plugin/tui`.
- `@uthakkan-dalam/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@uthakkan-dalam/plugin/tui"
...
- The value is a semver range checked against the running Dalam version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.dalam` is absent, no check is performed (backward compatible).
...
- `dalam plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `dalam plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.dalam` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.dalam`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.dalam`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.dalam` dir or `DALAM_CONFIG_DIR`, Dalam installs `@uthakkan-dalam/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@uthakkan-dalam/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Dalam's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Dalam registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Dalam pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@uthakkan-dalam/plugin/tui`.
...
- `message` is required; `title` defaults to `"dalam"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `dalam.default`.
...
- Local plugins persist installed themes under the local `.dalam/themes` area near the plugin config source.
...
- `--pure` / `DALAM_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.dalam/plugins/tui-smoke.tsx`
- Local vim plugin: `.dalam/plugins/tui-vim.tsx`
- Local smoke config: `.dalam/tui.json`
- Local smoke theme: `.dalam/plugins/smoke-theme.json`
