...
Practical reference for a future `packages/server` split after the dalam
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/dalam`.
...
Until enough shared service code lives outside `packages/dalam`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/dalam`

It should not import `packages/dalam` services while `packages/dalam`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on dalam-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/dalam` implementation details.
