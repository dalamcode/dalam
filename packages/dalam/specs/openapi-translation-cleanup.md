...
Trim `packages/dalam/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/dalam/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/dalam`.
- `[x]` `bun typecheck` from `packages/dalam`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/dalam/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/dalam`.
- `[x]` `bun dev generate > /tmp/dalam-openapi.json` from `packages/dalam`.
...
- Edit `packages/dalam/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/dalam-openapi.json` from `packages/dalam`.
...
- Check source schemas in files like `packages/dalam/src/session/schema.ts`, `packages/dalam/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/dalam/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/dalam/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
