...
Delete `packages/dalam/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/dalam/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/dalam/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/dalam`
- TUI plugin loader tests in `packages/dalam` if runtime plugin API wiring changed
