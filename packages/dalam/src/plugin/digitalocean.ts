import type { Hooks, PluginInput } from "@uthakkan/plugin"
import type { Model } from "@uthakkan/sdk/v2"
import { InstallationVersion } from "@uthakkan/core/installation/version"
import { OauthCallbackPage } from "@uthakkan/core/oauth/page"
...
async function listRouters(
  ...
  const res = await fetch(`${DO_GENAI_API}/models/routers`, {
    headers: {
      ...
      "User-Agent": `dalam/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              url,
              instructions:
                "Sign in to DigitalOcean in your browser. Dalam will use your DigitalOcean API token directly for inference and load your Inference Routers. Re-run /connect to refresh routers later.",
