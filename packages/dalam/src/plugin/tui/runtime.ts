...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@uthakkan/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@uthakkan/core/effect/app-node-builder"
import { errorData, errorMessage } from "@uthakkan/tui/util/error"
...
import { hasTheme, upsertTheme } from "@uthakkan/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@uthakkan/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@uthakkan/core/v1/config/plugin"
import { createCommandShim } from "@uthakkan/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@uthakkan/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".dalam"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".dalam", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".dalam", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.DALAM_PURE ? [] : pluginOrigins
    if (Flag.DALAM_PURE && pluginOrigins.length) {
    }
