...
export const DEPRECATED_PLUGIN_PACKAGES = ["dalam-openai-codex-auth", "dalam-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, dalamVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(dalamVersion) || semver.major(dalamVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.dalam
  if (typeof range !== "string") return
  if (!semver.satisfies(dalamVersion, range)) {
    throw new Error(`Plugin requires dalam ${range} but running ${dalamVersion}`)
  }
}
