...
import { isRecord } from "@uthakkan/tui/util/record"
...
import { TuiKeybind } from "@uthakkan/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@dalam/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old dalam.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.dalam`
  // folders between cwd and home, and DALAM_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.DALAM_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit DALAM_TUI_CONFIG override, if set.
  if (Flag.DALAM_TUI_CONFIG) {
    const configFile = Flag.DALAM_TUI_CONFIG
  ...
  // 4. `.dalam` directories (and DALAM_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".dalam") || dir === Flag.DALAM_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".dalam") && dir !== Flag.DALAM_CONFIG_DIR) continue
