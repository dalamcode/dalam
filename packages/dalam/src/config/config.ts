...
import { InstallationLocal, InstallationVersion } from "@uthakkan/core/installation/version"
...
import { EffectFlock } from "@uthakkan/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@uthakkan/core/v1/config/config"
import { RemoteAuthError } from "@uthakkan/core/v1/config/error"
import { ConfigPermissionV1 } from "@uthakkan/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@dalam/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["dalam.jsonc", "dalam.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://dalam.uthakkan.in/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.DALAM_CONFIG && !Flag.DALAM_CONFIG_DIR && !Flag.DALAM_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://dalam.uthakkan.in/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "dalam.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "dalam.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://dalam.uthakkan.in/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "DALAM_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/dalam`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://dalam.uthakkan.in/config.json"
        ...
        if (Flag.DALAM_CONFIG) {
          yield* merge(Flag.DALAM_CONFIG, yield* loadFile(Flag.DALAM_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.DALAM_CONFIG })
        }

        if (!Flag.DALAM_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("dalam", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.DALAM_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from DALAM_CONFIG_DIR", { path: Flag.DALAM_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".dalam") || dir === Flag.DALAM_CONFIG_DIR) {
            for (const file of ["dalam.json", "dalam.jsonc"]) {
          ...
          // Auto-discovered plugins under `.dalam/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.DALAM_CONFIG_CONTENT) {
          const source = "DALAM_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.DALAM_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from DALAM_CONFIG_CONTENT")
        }
        ...
        if (Flag.DALAM_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.DALAM_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("DALAM_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.DALAM_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.DALAM_DISABLE_PRUNE) {
