...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from dalam.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const dalam = await dalamFiles(input)
  for (const file of dalam) {
...
async function dalamFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "dalam"),
    ...(await Filesystem.findUp(["dalam.json", "dalam.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
