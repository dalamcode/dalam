...
import type { AssistantMessage, Message, DalamClient, SessionMessageResponse } from "@uthakkan/sdk/v2"
...
export const AuthMethodID = "dalam-login"
...
export class Service extends Context.Service<Service, Interface>()("@dalam/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `dalam auth login` in the terminal",
      name: "Login with dalam",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: DalamClient) {
...
function makeUsageService(sdk: DalamClient) {
...
async function loadDirectorySnapshot(sdk: DalamClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, dalam provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const dalamProvider = providers[ProviderV2.ID.make("dalam")]
  const dalamModel = dalamProvider ? Provider.sort(Object.values(dalamProvider.models))[0] : undefined
  if (dalamProvider && dalamModel) return { providerID: dalamProvider.id, modelID: dalamModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Dalam service failure", service })
}
