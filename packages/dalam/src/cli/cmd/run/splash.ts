...
//
// Renders the full dalam entry logo and a compact [O] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    const markLines = mark.slice(1)
    const top = 1
    const body_left = (markLines[0]?.length ?? 0) + 2

    for (let i = 0; i < markLines.length; i += 1) {
      draw(lines, markLines[i] ?? "", {
        left: 0,
        top: top + i,
        fg: left,
        shadow: leftShadow,
    ...
    push(lines, body_left, top, "Dalam", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
