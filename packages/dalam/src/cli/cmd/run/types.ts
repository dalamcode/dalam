// Shared type vocabulary for the direct interactive mode (`dalam --mini`).
//
...
import type { DalamClient, PermissionRequest, QuestionRequest, ToolPart } from "@uthakkan/sdk/v2"
import type { TuiConfig } from "@uthakkan/tui/config"
...
type PromptModel = Parameters<DalamClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<DalamClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<DalamClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<DalamClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<DalamClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<DalamClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<DalamClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<DalamClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<DalamClient["question"]["reject"]>[0]
