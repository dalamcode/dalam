...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall dalam and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g uthakkan-dalam",
      pnpm: "pnpm uninstall -g uthakkan-dalam",
      bun: "bun remove -g uthakkan-dalam",
      yarn: "yarn global remove uthakkan-dalam",
      brew: "brew uninstall dalam",
      choco: "choco uninstall dalam",
      scoop: "scoop uninstall dalam",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "dalam", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Dalam!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# dalam") || content.includes(".dalam/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".dalam/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".dalam/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".dalam"))
    ) {
      continue
    }
