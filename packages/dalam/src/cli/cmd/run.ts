import type { PermissionV1 } from "@uthakkan/core/v1/permission"
import { FSUtil } from "@uthakkan/core/fs-util"
// CLI entry point for `dalam run` and `dalam --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`dalam --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`dalam --mini --attach`): connects to a running
//      dalam server and runs interactive mode against it.
//
...
import { createDalamClient, type DalamClient, type ToolPart } from "@uthakkan/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<DalamClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run dalam with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running dalam server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to DALAM_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to DALAM_SERVER_USERNAME or 'dalam')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: DalamClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: DalamClient, sessionID: string) {
      ...
      async function current(sdk: DalamClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: DalamClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: DalamClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: DalamClient) {
        ...
        async function loop(client: DalamClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createDalamClient({
        baseUrl: "http://dalam.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
