...
import { llmClient } from "@uthakkan/core/effect/app-node-platform"
...
import type { LLMEvent } from "@uthakkan/llm"
import { LLMClient } from "@uthakkan/llm/route"
import type { LLMClientService } from "@uthakkan/llm/route"
...
export class Service extends Context.Service<Service, Interface>()("@dalam/LLM") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      // from the workflow service are executed via dalam's tool system
      ...
      if (language instanceof GitLabWorkflowLanguageModel) {
        ...
        workflowModel.toolExecutor = async (toolName, argsJson, _requestID) => {
          ...
          try { parsedArgs = JSON.parse(argsJson) } catch { return { result: "", error: `Invalid JSON args: ${argsJson}` } }
          try {
      ...
      // Runtime seam: native is an opt-in adapter over @uthakkan/llm. It
