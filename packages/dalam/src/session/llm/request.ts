...
const USER_AGENT = `dalam/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const dalamProjectID = input.model.providerID.startsWith("dalam")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("dalam")
        ? {
            ...(dalamProjectID ? { "x-dalam-project": dalamProjectID } : {}),
            "x-dalam-session": input.sessionID,
            "x-dalam-request": input.user.id,
            "x-dalam-client": input.flags.client,
