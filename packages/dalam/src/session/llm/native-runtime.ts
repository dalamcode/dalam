...
import type { LLMClientShape } from "@uthakkan/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("dalam"))
    return { type: "unsupported", reason: "provider is not openai, dalam, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @uthakkan/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains dalam-owned. The native runtime only adapts
      // the @uthakkan/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
