...
import { ModelsDevPlugin } from "@uthakkan/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.DALAM_MODELS_PATH,
          disabled: Flag.DALAM_DISABLE_MODELS_FETCH,
        }
        Flag.DALAM_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.DALAM_MODELS_PATH = previous.path
          Flag.DALAM_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
