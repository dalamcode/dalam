import { AISDK } from "@uthakkan/core/aisdk"
import { describe, expect, mock } from "bun:test"
import { Effect } from "effect"
import { ModelV2 } from "@uthakkan/core/model"
import { PluginV2 } from "@uthakkan/core/plugin"
import { PluginHost } from "@uthakkan/core/plugin/host"
import { CloudflareAIGatewayPlugin } from "@uthakkan/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            ...
            metadata: { invoked_by: "test", project: "dalam" },
            cacheTtl: 300,
            cacheKey: "cache-key",
            skipCache: true,
            collectLog: false,
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("dalam/"),
            },
          },
        })
      }),
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            name: "cloudflare-ai-gateway",
            headers: {
              "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "dalam" }),
            },
          },
        })

        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "dalam" },
        })
      }),
    ),
  )
