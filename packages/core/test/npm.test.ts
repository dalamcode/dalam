...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@dalam/acme")).toBe("@dalam/acme")
    expect(Npm.sanitize("@dalam/acme@1.0.0")).toBe("@dalam/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/dalam/acme.git"
    const expected = win ? "acme@git+https_//github.com/dalam/acme.git" : spec
