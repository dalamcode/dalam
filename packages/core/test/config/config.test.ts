...
import { Policy } from "@uthakkan/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads dalam JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "dalam.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "dalam.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "dalam.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "dalam.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "dalam.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "dalam.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .dalam configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".dalam"), { recursive: true })
            await fs.mkdir(path.join(directory, ".dalam"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "dalam.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "dalam.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "dalam.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "dalam.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "dalam.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".dalam", "dalam.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".dalam", "dalam.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".dalam")),
              AbsolutePath.make(path.join(directory, ".dalam")),
            ])
