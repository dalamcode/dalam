...
import { ConfigProviderPlugin } from "@uthakkan/core/config/plugin/provider"
...
describe("ConfigProviderPlugin.Plugin", () => {
  it.effect("keeps configured model variant bodies unchanged", () =>
    Effect.gen(function* () {
      ...
      const providerID = ProviderV2.ID.dalam
          ...
          Effect.succeed([
            new Config.Document({
              ...
              info: decode({
                providers: {
                  dalam: {
                    api: { type: "aisdk", package: "@ai-sdk/openai", url: "https://dalam.test/v1" },
                    models: {
                      "alpha-gpt-next": {
                        variants: [
                          {
