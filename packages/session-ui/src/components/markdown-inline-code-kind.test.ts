...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@uthakkan/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/dalam`)).toBe("path")
    expect(inlineCodeKind(`@uthakkan/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://dalam.uthakkan.in/docs`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/dalam`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://dalam.uthakkan.in/docs`)).toBeUndefined()
  })
})
