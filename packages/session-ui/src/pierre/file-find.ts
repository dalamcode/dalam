...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("dalam-find")
  api.delete("dalam-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("dalam-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("dalam-find", new Highlight(...rest))
    return true
  }
