...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Dalam launch", () => {
    expect(hasExistingAppState([file("dalam.settings")])).toBe(true)
    expect(hasExistingAppState([file("dalam.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("dalam")])).toBe(true)
  })
})
