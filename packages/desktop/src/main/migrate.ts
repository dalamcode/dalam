...
const TAURI_APP_IDS: Record<string, string> = {
  ...
  prod: "ai.dalam.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.dalam.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `dalam.settings.dat` is special: it maps to the `dalam.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"dalam.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // dalam.settings.dat → the electron settings store ("dalam.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "dalam.global.dat").
  const storeName = filename === "dalam.settings.dat" ? "dalam.settings" : filename
