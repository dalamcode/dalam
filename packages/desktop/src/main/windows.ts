...
import { resolveThemeVariant } from "@uthakkan/ui/theme/resolve"
import type { DesktopTheme } from "@uthakkan/ui/theme/types"
...
function windowDataFile(id: string) {
  return `dalam.window.${id.replace(/[^a-zA-Z0-9._-]/g, "-")}.dat`
}
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "Dalam window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("Dalam is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
