...
const TEST_ONBOARDING = process.env.DALAM_TEST_ONBOARDING === "1"
...
const main = Effect.gen(function* () {
  ...
  process.env.DALAM_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "in.uthakkan.dalam.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `dalam-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Dalam Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("dalam://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("dalam")
  registerRendererProtocol()
  setDockIcon()
  ...
  const port = yield* Effect.gen(function* () {
    const fromEnv = process.env.DALAM_PORT
    if (fromEnv) {
