...
import { type WslCommandLine, resolveWslDalam, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const dalam = await resolveWslDalam(distro)
  if (!dalam) throw new Error(`Dalam is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "dalam"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export DALAM_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export DALAM_CLIENT=desktop",
    `export DALAM_SERVER_USERNAME=${shellEscape(username)}`,
    `export DALAM_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(dalam)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
