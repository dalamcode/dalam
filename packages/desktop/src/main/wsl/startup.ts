...
export function expectDalamVersion(installed: string | null, expected: string, distro = "Debian") {
  if (installed === expected) return
  throw new Error(
    `Dalam update finished but ${distro} still reports ${installed ?? "no version"}; expected ${expected}`,
  )
}
