...
test("restarts an existing distro server after updating Dalam", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.dalam/bin/dalam",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, dalamChecks: {} })
})
...
test("ignores stale background Dalam checks after removing a WSL server", async () => {
  ...
  expect(controller.getState().dalamChecks).toEqual({})
})
...
test("probes addable distros in parallel before checking Dalam", async () => {
  persistedServers = []
  ...
  expect(dalam).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(dalam).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().dalamChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Dalam in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const dalam: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(dalam).toEqual(["Debian"])
  expect(Object.keys(controller.getState().dalamChecks)).toEqual(["Debian"])
})
