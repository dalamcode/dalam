...
export function createWslServersController(
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const dalamChecks = await Promise.all(
      unique
  ...
  const refreshDalamCheckBackground = (id: string, distro: string) => {
    void checkDalam(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl dalam check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkDalam(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl dalam check failed", {
              id: item.config.id,
  ...
  return {
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.dalamChecks, distro) : {}),
      })
    },
...
function dalamCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "dalam is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "dalam is installed but could not run",
    }
  }
  return {
    distro,
