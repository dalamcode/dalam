...
import type { UpdaterState } from "@uthakkan/app/updater"
...
const deepLinkEvent = "dalam:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__DALAM__ ??= {}
  const pending = window.__DALAM__.deepLinks ?? []
  window.__DALAM__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `dalam.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("dalam.global.dat").getItem("language")
