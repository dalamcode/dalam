...
test("keeps a hidden prod launcher for old Linux pins", async () => {
  ...
  expect(config.deb?.fpm?.[0]).toEndWith(`${legacyDesktopEntry}=/usr/share/applications/in.uthakkan.dalam.desktop`)
  expect(config.rpm?.fpm?.[0]).toEndWith(`${legacyDesktopEntry}=/usr/share/applications/in.uthakkan.dalam.desktop`)
