...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      DALAM_CLI_NAME: `'${binary}'`,
      DALAM_MODELS_DEV: modelsData,
      DALAM_CHANNEL: `'${Script.channel}'`,
      DALAM_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/dalamcode/dalam.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
